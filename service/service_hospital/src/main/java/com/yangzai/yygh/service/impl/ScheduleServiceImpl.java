...
import com.yangzai.yygh.entity.BookingRule;
...
import com.yangzai.yygh.repository.ScheduleRepository;
...
import com.yangzai.yygh.vo.BookingScheduleRuleVo;
...
public class ScheduleServiceImpl implements ScheduleService
{
    ...
    public Map<String, Object> getScheduleRule(long page, long limit, String hoscode, String depcode) {
        ...
        //根据工作日workDate进行分组 Aggregation在mongodb中用于聚合操作(分组等操作)
        Aggregation aggregation = Aggregation.newAggregation(
                Aggregation.match(criteria), //匹配条件
                Aggregation.group("workDate")//分组字段
                .first("workDate").as("workDate")  //分组取名
                //统计号源数量（就诊的医生数量）
                .count().as("docCount")
        ...
        //调用方法最终查询
        AggregationResults<BookingScheduleRuleVo> results = mongoTemplate
                .aggregate(aggregation, Schedule.class, BookingScheduleRuleVo.class);
        List<BookingScheduleRuleVo> mappedResults = results.getMappedResults();
        ...
        result.put("bookingScheduleList", mappedResults);
...
//        Schedule schedule = scheduleRepository.findById(scheduleId).get();
        Schedule schedule = scheduleRepository.findByHosScheduleId(scheduleId);
        if(schedule == null) throw new YyghException(
                ResultCodeEnum.PARAM_ERROR
        );
        //获取预约规则的信息
        Hospital hospital = hospitalService.getByHoscode(schedule.getHoscode());
        if(hospital == null) throw new YyghException(
                ResultCodeEnum.PARAM_ERROR
        );
        BookingRule bookingRule = hospital.getBookingRule();
        if(bookingRule == null) throw new YyghException(
                ResultCodeEnum.PARAM_ERROR
        );

        //把获取的数据保存到scheduleOrderVo中
        ...
        //预约截止时间
        DateTime endTime = this.getDateTime(new DateTime().plusDays(bookingRule.
                getCycle()).toDate(), bookingRule.getStopTime());
    ...
    private IPage getListDate(Integer page, Integer limit, BookingRule bookingRule) {
        ...
        if (releaseTime.isBeforeNow()) cycle ++;
        //获取可预约所有日期，最后一天显示即将放号
        List<Date> dateList = new ArrayList<>();
        for(int i = 0 ; i < cycle; i ++){
            DateTime curDateTime = new DateTime().plusDays(i); //当前时间加一天
            String dateString = curDateTime.toString("yyyy-MM-dd");
            dateList.add(new DateTime(dateString).toDate());
        }
        //因为预约的周期不同，每页显示日期最多七天的数据，超过七天进行分页
        List<Date> pageDateList = new ArrayList<>();
        int start = (page - 1)*limit;
        int end = (page - 1)*limit + limit;
        //如果可以显示的数据小于7条 直接显示
        if(end > dateList.size()) end = dateList.size();
        for(int i = start ; i < end ; i ++) pageDateList.add(dateList.get(i));
        //如果大于7 进行分页
